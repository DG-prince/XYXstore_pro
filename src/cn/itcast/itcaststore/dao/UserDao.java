...
public class UserDao {
    ...
    public Boolean addUser(User user) throws SQLException{
    	String sql="insert into user(id,name,password,money,email,role) values(?,?,?,?,?,?)";
    	QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
    	
    	int count = runner.update(sql,user.getId(),user.getName(),
    			user.getPassword(),10000,user.getEmail(),"普通用户");
    ...
    public Boolean addSaler(User user) throws SQLException{
    	String sql="insert into user(id,name,password,email,gtype,role) values(?,?,?,?,?,?)";
    	QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
    	
    	int count = runner.update(sql,user.getId(),user.getName(),
    			user.getPassword(),user.getEmail(),user.getgType(),"销售");
